...
def get_performance(model, Tm, Tn, Tr, Tc, Tk, W_p, I_p, O_p,device=None, ignored_layers = []):
        ...
        if isinstance(layer, nn.Conv2d) or isinstance(layer,copy_conv2d.Conv2d_Custom):
            ...
            if T == "cconv":
                ...
                if acc_1.Success==False:
                    ...
                    if not (layer_name in ignored_layers):
                        cTT += perf[0]
...
def get_performance_layer(layer, Tm, Tn, Tr, Tc, Tk, W_p, I_p, O_p,device=None, size = None):
    input = torch.Tensor(size).to(torch.float32)
    cTT = 0
