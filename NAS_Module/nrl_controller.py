...
from model_search_space import ss_mnasnet1_0, ss_mnasnet0_5, ss_resnet18, ss_mobilenet_v2, ss_proxyless_mobile, ss_resnet18_smaller
...
class Controller(object):
    ...
    def __init__(self):
        ...
        if self.args.model == "newresnet18":
            self.nn_model_helper = ss_resnet18_smaller
    ...
    def child_network_translate(self, child_network):
        ...
        for para_idx in range(self.num_para):
            ...
            dnn_out[0].append(self.para_2_val[para_idx][child_network[0][para_idx]])
        return dnn_out
    ...
    def generate_child_network(self, child_network_architecture):
        with self.graph.as_default():
            ...
            for para_idx, prob in rnn_out.items():
                if self.args.random:
                    predict_child[0][para_idx] = np.random.choice(range(len(self.para_2_val[para_idx])))
                else:
    ...
    def generate_child_network_random(self, child_network_architecture):
        with self.graph.as_default():
            feed_dict = {
                self.child_network_paras: child_network_architecture,
                self.batch_size: 1
    ...
    def global_train(self):
        ...
        for episode in range(controller_params['max_episodes']):
            ...
            for sub_child in range(controller_params["num_children_per_episode"]):
                ...
                if self.args.model == "newresnet18":
                    Para_NN1 = Para_NN1[0] + Para_NN1[1:]
                print(Para_NN1)
                print(DNA_NN1)
                # exit()
